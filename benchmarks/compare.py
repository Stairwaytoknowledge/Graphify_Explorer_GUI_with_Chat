...
Upstream graphify CLI: https://github.com/safishamsi/graphify (Safi Shamsi).
This script measures the wrapper's UX overhead on top of it.

What it measures (numbers, not opinions):
...
def main() -> int:
    ...
    md.append("> The wrapper this measures is a desktop UI around the")
    md.append("> upstream graphify CLI by")
    md.append("> [Safi Shamsi](https://github.com/safishamsi/graphify).")
    md.append("> The graph engine itself is upstream's; the wrapper only")
    md.append("> adds packaging, the GUI, and a local Ollama chat layer.\n")
    md.append("Generated by `benchmarks/compare.py`. Numbers below are from a")
    md.append("single run on a single machine; rerun the script to refresh.\n")
