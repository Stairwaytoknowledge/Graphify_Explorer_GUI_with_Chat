...
def main() -> int:
    ...
    # Anonymize the path so no committed report leaks home/username.
    try:
        repo_anon = args.repo.relative_to(Path.home())
        repo_display = "~/" + str(repo_anon).replace("\\", "/")
    except ValueError:
        repo_display = args.repo.name
    md.append(f"- Test repo: `{repo_display}`")
