...
The graph and `graphify query` BFS that this eval reads are produced by
the upstream graphify CLI (https://github.com/safishamsi/graphify by
Safi Shamsi). The chat layer being evaluated is what this wrapper adds.

Runs a fixed set of questions against a fixed test repo, in both Fast and
Quality retrieval modes, with one or more local Ollama models. Reports:
...
def main() -> int:
    ...
    md.append("> Evaluates the chat layer on top of upstream graphify by")
    md.append("> [Safi Shamsi](https://github.com/safishamsi/graphify).")
    md.append("> The graph and BFS results come from upstream's CLI; the")
    md.append("> retrieval planner and answerer are what this wrapper adds.\n")
    md.append("Generated by `benchmarks/chat_eval.py`. Single run on a "
              "single machine; rerun for fresh numbers.\n")
